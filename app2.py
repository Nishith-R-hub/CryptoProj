...
def step_3_consent(username):
    ...
    while True:
        ...
        if decision in ["y", "yes"]:
            ...
            success("User approved the requested permissions.")
            return True

        if decision in ["n", "no"]:

            error("User denied the requested permissions.")
            return False
...
def step_4_authorization_code(username):
    ...
    # --------------------------------------------------------
    # State round-trip simulation
    # --------------------------------------------------------

    print(f"{BOLD}State Round-Trip Verification{RESET}")
    line("-")
...
def exchange_code_for_token(code):
    ...
    print(f"{BOLD}1. Client Authentication{RESET}")
    line("-")
...
def inspect_store():
    ...
    title("LIVE OAUTH STORE INSPECTOR")
    ...
    print()
    print(
        f"{BOLD}AUTHORIZATION CODES{RESET}"
    )
    ...
    if not ACCESS_TOKENS:
        ...
        for index, (token, data) in enumerate(
            ACCESS_TOKENS.items(),
            start=1
        ):
    ...
    print(
        f"Users: {', '.join(USERS.keys())}"
    )

    print(
        f"Current timestamp: {int(now)}"
    )
...
def test_state():
    ...
    title("SECURITY TEST — STATE PARAMETER")
    ...
    if original_state == returned_state:
        ...
        print(
            "The callback state matches the original request."
        )
...
def test_single_use():
    ...
    title("SECURITY TEST — SINGLE-USE CODE")
    ...
    if AUTH_CODES[code]["used"]:
        ...
        print(
            "Single-use protection is working."
        )
...
def test_ttl():
    ...
    title("SECURITY TEST — AUTHORIZATION CODE TTL")
    ...
    print(
        "Simulated expiration: TRUE"
    )

    print()

    if time.time() > AUTH_CODES[code]["expires_at"]:
...
# ============================================================
# SECURITY TEST — CLIENT AUTHENTICATION
# ============================================================
...
def test_client_authentication():
    ...
    title("SECURITY TEST — CLIENT AUTHENTICATION")
    ...
    if authenticate_client(
        ...
        success(
            "Correct client credentials accepted."
        )
    ...
    if not authenticate_client(
        ...
        success(
            "Incorrect client credentials rejected."
        )
...
def test_redirect():
    ...
    title("SECURITY TEST — REDIRECT URI BINDING")

    print()

    valid_redirect = REDIRECT_URI
    ...
    print(
        f"Registered URI : {valid_redirect}"
    )

    print(
        f"Test URI       : {invalid_redirect}"
    )

    print()

    if invalid_redirect not in CLIENT["redirect_uris"]:
        ...
        print(
            "The authorization request cannot redirect "
            "to an unregistered destination."
        )

    else:

        error(
            "Redirect URI test failed."
        )
...
def security_tests():
    ...
    while True:
        ...
        print(
            "2. Single-use authorization code"
        )

        print(
            "3. Authorization code TTL"
        )
...
def report():
    ...
    title("BCS703 — OAUTH2 VIRTUAL LAB REPORT")
    ...
    print(
        f"{BOLD}Authorization Code Grant Flow{RESET}"
    )
    ...
    print(
        f"{BOLD}Security Properties Demonstrated{RESET}"
    )
    ...
    print(
        "It is a carefully ordered authorization protocol"
    )

    print(
        "whose safety depends on TTL, single-use codes,"
    )

    print(
        "authenticating the Client at the token endpoint,"
    )

    print(
        "binding the callback with state, and treating"
    )

    print(
        "access tokens as secrets."
    )
